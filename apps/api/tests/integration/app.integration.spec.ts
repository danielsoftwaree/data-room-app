...
import { DRIZZLE } from '../../src/config/database/database.tokens';
import { DATAROOMS_REPOSITORY } from '../../src/modules/datarooms/domain/datarooms.repository.port';
...
/**
 * Integration coverage over the real HTTP layer: controllers, DTO validation, multipart
...
describe('folders and nodes API', () => {
  ...
  test('filters nodes by case-insensitive name substring', async () => {
    const dataroom = await createDataroom('Searchable');
    const folder = (await (
      await json(`/datarooms/${dataroom.id}/folders`, 'POST', {
        parentId: null,
        name: 'Financials',
      })
    ).json()) as { id: string };
    await api(`/datarooms/${dataroom.id}/files`, {
      method: 'POST',
      body: pdfFormData('Q1 Balance Sheet.pdf', folder.id),
    });
    await api(`/datarooms/${dataroom.id}/files`, {
      method: 'POST',
      body: pdfFormData('Legal Summary.pdf'),
    });

    const matches = (await (
      await api(`/datarooms/${dataroom.id}/nodes?search=balance`)
    ).json()) as { name: string }[];
    expect(matches.map((node) => node.name)).toEqual(['Q1 Balance Sheet.pdf']);

    const empty = (await (
      await api(`/datarooms/${dataroom.id}/nodes?search=missing`)
    ).json()) as unknown[];
