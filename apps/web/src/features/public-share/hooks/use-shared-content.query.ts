 ...
 * generated `getSharedFileContent` caller instead of a GET query hook.
 *
 * The bytes for a (slug, password) pair never change, so the query is cached
 * forever — the object URL stays stable and pdf.js does not re-render.
 */
export function useSharedContent(slug: string, password: string) {
  const content = useQuery({
    ...
    queryKey: ['public-share', slug, 'content'],
    queryFn: () => getSharedFileContent(slug, { password }),
    staleTime: Infinity,
    retry: false,
  });
