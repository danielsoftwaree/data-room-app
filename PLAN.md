...
- README-путь запуска: `docker compose up -d` → `bun install` → `bun run dev`
  (миграции применяются на старте api сами — works out of the box).
...
- `apps/api/src/config/env/env.ts` — безопасные dev-defaults для `DATABASE_URL` и pool size.
- `apps/api/src/config/database/*` — Nest DI (`DatabaseModule`), pg `Pool`,
  `drizzle(pool, { schema })`, programmatic migrations и `onApplicationShutdown`.
...
`db:*` — скрипты `packages/db`; `apps/api` оставляет прокси-скрипты для удобства.
...
in-memory фейков репозитория и стораджа (20), unit `ApiExceptionFilter` (3), HTTP e2e (14) —
...
2. Зависимости: `packages/db` владеет `drizzle-orm@0.45.2` + `drizzle-kit@0.31.10`;
   `apps/api` владеет runtime wiring (`pg@8.22.0`, `@types/pg`).
3. `packages/db/drizzle.config.ts` + `packages/db/src/schema.ts` +
   `apps/api/src/config/database/DatabaseModule`/`DatabaseMigrator`.
...
3. ✅ **PostgreSQL + Drizzle ORM** — durable storage вместо `Map`; миграции на старте API.
...
10. **README + деплой** — design decisions, setup (docker compose + автомиграции), hosted URL.
