...
import type { SharedFileDto } from '@repo/contracts';
import { Button } from '@repo/ui/components/button';
import { EmptyState } from '@repo/ui/components/empty-state';
import { Input } from '@repo/ui/components/input';
import { Label } from '@repo/ui/components/label';
import { Skeleton } from '@repo/ui/components/skeleton';
import { FilePdfIcon, VaultIcon } from '@phosphor-icons/react';
...
/**
 * The public, unauthenticated page behind a `/share/:slug` link. It has two
 * states, held entirely in component state so the password never touches the
 * URL or storage: the password gate, then — once unlocked — the file preview
 * and download. A dead/removed link (404) replaces the gate with an empty state.
 */
export function PublicShareScreen({ slug }: Readonly<{ slug: string }>) {
  const [unlocked, setUnlocked] = useState<{ password: string; file: SharedFileDto } | null>(null);

  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-4 py-10">
      <ProductMark />
      {unlocked ? (
        <UnlockedView slug={slug} password={unlocked.password} file={unlocked.file} />
      ) : (
        <UnlockGate slug={slug} onUnlocked={(password, file) => setUnlocked({ password, file })} />
      )}
    </main>
  );
}
...
function UnlockGate({
  slug,
  onUnlocked,
}: Readonly<{ slug: string; onUnlocked: (password: string, file: SharedFileDto) => void }>) {
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Enter the password you were given to view and download it.
          </p>
        </div>
      </div>
...
/** Unlocked: file header + download, and an inline continuous-scroll PDF preview. */
function UnlockedView({
  slug,
  password,
  file,
}: Readonly<{ slug: string; password: string; file: SharedFileDto }>) {
  const content = useSharedContent(slug, password);
  ...
  return (
    <div className="w-full max-w-3xl overflow-hidden rounded-xl border bg-card shadow-sm">
      <header className="flex flex-wrap items-center gap-3 border-b p-4">
        ...
        {content.objectUrl ? (
          <Button asChild>
            <a href={content.objectUrl} download={file.name}>
